...
int main(int argc, char *argv[]) {
    try {
        ...
        const std::string lrInputBlobName = inputInfo.begin()->first;
        /** Collect images**/
        std::vector<cv::Mat> inputImages;
        for (const auto &i : imageNames) {
            /** Get size of low resolution input **/
            auto lrInputInfoItem = inputInfo.begin()->second;//inputInfo[lrInputBlobName];
            ...
            slog::info << "Collect and load image " << slog::endl;
        ...
        slog::info << "PrepareInput Finish" << slog::endl;
        // -----------------------------------------------------------------------------------------------------
        ...
        std::cout << "Inference Time :" << (end - start)  << "ms"<< std::endl;
        // -----------------------------------------------------------------------------------------------------
        ...
        for (size_t i = 0; i < numOfImages; ++i) {
            ...
            if (numOfChannels == 3) {
                ...
                // read BGR image
                cv::Mat mat = cv::imread(imageNames[i], cv::IMREAD_COLOR);

                // BGR2YCrCb and bicubic
                cv::Mat mat_ycrcb, mat_out_ycrcb;
                cv::cvtColor(mat, mat_ycrcb, cv::COLOR_BGR2YCrCb);
                cv::resize(mat_ycrcb, mat_out_ycrcb, cv::Size(h, w), 0, 0, cv::INTER_CUBIC);

                // replace Y
                std::vector<cv::Mat> channels;
                cv::split(mat_out_ycrcb, channels);
                channels[0] = imgPlanes[0];
                ...
                if (FLAGS_show) {
                    cv::imshow("result", mat_out_bgr);
                    cv::waitKey();
                }

                std::string outImgName = std::string("sr_" + std::to_string(i + 1) + ".png");
                cv::imwrite(outImgName, mat_out_bgr);
            }
